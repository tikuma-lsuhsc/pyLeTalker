...
def calc_stress(p):
    # Stress calculations for the three-mass model based on Titzende and Story
    # 2002
    # Author: Brad Story
    ...
    sig0 = np.array([5000.0, 4000, 10000])
    sig2 = np.array([300000, 13930, 15000])

    Ce = np.array([4.4, 17, 6.5])

    strs = (-sig0 / ep1) * (p.eps - ep1)

    tf = p.eps > ep2
    if tf.any():
        sig_nln = sig2[tf] * (
            np.exp(Ce[tf] * (p.eps - ep2[tf])) - 1 - Ce[tf] * (p.eps - ep2[tf])
        )
        strs[tf] += sig_nln

    p.sigmuc, p.sigl, p.sigp = strs

    return p

def rules_consconv(p, c):
    ...
    p.kr = 0.5 * c.muc * EffL * p.T * p.Dc + (pi ** 2 * p.ic * p.sigc) / (
        EffL ** 2 * c.rho
    )

    p.M = c.rho * EffL * p.T * p.Db  # mass of body
    p.K = (
        2 * c.mub * EffL * p.T / p.Db + pi ** 2 * p.sigb * p.Db * p.T / EffL
    )  # stiffness of body

    p.k1 = (
        2 * c.muc * (EffL * p.T / p.Dc) * p.zn / p.T + pi
        ** 2 * p.sigc * (p.Dc / EffL) * p.zn
    )  # (12)
    p.k2 = 2 * c.muc * (EffL * p.T / p.Dc) * (1 - p.zn / p.T) + pi ** 2 * p.sigc * (
        p.Dc / EffL
    ) * p.T * (
        1 - p.zn / p.T
    )  # (13)
    ...
    p.kc = (
        (
            0.5 * c.muc * (EffL * p.Dc / p.T) / (1 / 3 - a * (1 - a))
            - 2 * c.muc * (EffL * p.T / p.Dc)
        )
        * a
        * (1 - a)
    )
    ...
    p.bc = (
        2 * c.zeta * sqrt(p.ic * p.kr)
    )  # Check to see if the same zeta is used for rot. damping
