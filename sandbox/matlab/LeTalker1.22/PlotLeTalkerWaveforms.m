function PlotLeTalkerWaveforms(r, p, Fs, hwaves, htract);
    ...
    if (max(r.po) > 0 && min(r.po) < 0)
        ...
        hold on
        plot(t, r.ps, '-k', 'LineWidth', 1);
        plot(t, filtfilt(b, a, r.ps), '-r')
        %p_intrag = ((r.f1*p.T*p.L)+(r.f2*p.T*p.L))/2;
        p_intrag = (r.f1 + r.f2) / 2 * 2 * p.T * p.L;
        plot(t, p_intrag, '-b');
        axis([t(1) t(end) min(p_intrag) - 1000 max(r.ps) + 1000])
        zeroline('-k')
        set(gca, 'Box', 'off',  'FontSize', 12, 'FontWeight', 'bold');
        v = axis;
        text(-.0025 * N, (v(3) + v(4)) / 2, 'Ps && Pg', 'FontSize', 12, 'FontWeight', 'bold')
        h = legend('Ps', 'Ps (smoothed)', 'Pg');

        % -------Plot glottal flow -----------
        subplot(5, 1, 3)
        plot(t, r.ug, '-k', 'LineWidth', 1);
        ...
        title(['Max Glottal Flow = ' num2str(round(max(r.ug))) ' cm^3/s']);

        % -------Plot glottal area -----------
        subplot(5, 1, 4)
        plot(t, r.ga, '-k', 'LineWidth', 1);
